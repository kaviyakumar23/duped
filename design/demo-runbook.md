...
| 0:20–0:50 | **★ SEE THE BUG, THEN SEE IT FIXED** (the killer beat) | On `/try`, the side-by-side: click **Run the trade race** on the NAIVE economy → the one legendary multiplies to **× ~20** in red ("the sword now exists in 20 inventories — real rows, real SQL"). Then click **Run the SAME race** on DUPED → it stays a calm gold **× 1**. "Same attack. Same concurrency. One database lets it dupe; the other makes it unrepresentable." This is the whole pitch in 30 seconds. |
| 0:50–1:05 | **The world** | Pan the live arena: ONE glowing legendary with its owner, Tokyo + Seoul nodes, the whale. Point at the calm **× 1 / ONE LEGENDARY**. |
